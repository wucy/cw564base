...
cmd = 'qsub -o %s -j y -l qp=low -l generation=70 -t 1-%d -tc 100 -S /bin/bash base/tools/run-array-job.sh %s /usr/bin/python base/local/py/decode_mbt_one_task.py %s %s %s %s %s SET' % (logs, tot, lst_fn, TSET, MLP, CONFIG, NUM_BASIS, LAMFILE)
