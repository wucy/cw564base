...
if __name__ == '__main__':
    ...
    for i in range(1, num_basis + 1):
        merged_scp = basis_dir_prefix + str(i) + '/lib/flists_align/' + dataset + '.scp.new'
        os.popen('rm ' + merged_scp)
        os.popen('cat ' + basis_dir_prefix + str(i) + '/lib/flists_align/*.scp.new > ' + merged_scp)
        scp_dict[i] = parse_scp(merged_scp)
        mlf_dict[i] = parse_mlf(basis_dir_prefix + str(i) + '/lib/mlabs/' + dataset + '.state.mlf')
