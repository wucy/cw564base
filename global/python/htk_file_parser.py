...
def gen_scp_file(out_fn, scp_dict, order = None):
    if order == None:
...
def parse_mlf(mlf_fn):
    ...
    ret[header] = content #last one
    return ret

def gen_mlf_file(out_fn, mlf_dict, order = None):
    if order == None:
